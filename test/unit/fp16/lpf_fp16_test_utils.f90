...
MODULE lpf_fp16_test_utils
    USE iso_fortran_env, only: real32, real64
    USE LPF_FP16
    IMPLICIT NONE

    PRIVATE
    PUBLIC :: check_fp16_real64
    PUBLIC :: check_logical
    PUBLIC :: check_integer
    PUBLIC :: check_fp16_known_bug
    PUBLIC :: test_summary
    PUBLIC :: integer_to_char
    PUBLIC :: FP16_TOL
    PUBLIC :: FP16_TOL_TIGHT
    PUBLIC :: FP16_TOL_LOOSE

    ! FP16 has 10 mantissa bits (11 digits including implicit 1),
    ! giving ~0.05% precision, so 0.1% is a safe default tolerance
    real(real64), parameter :: FP16_TOL      = 1.0d-2
    real(real64), parameter :: FP16_TOL_TIGHT = 1.0d-3
    real(real64), parameter :: FP16_TOL_LOOSE = 5.0d-2
...
CONTAINS
    ...
    subroutine check_fp16_real64(name, got_fp16, expected_r64, tol)
        character(len=*), intent(in) :: name
        type(FP16), intent(in) :: got_fp16
        real(real64), intent(in) :: expected_r64
        real(real64), intent(in) :: tol

        real(real64) :: got_r64, diff, threshold

        got_r64 = dble(got_fp16)
    ...
    subroutine check_fp16_known_bug(name, got_fp16, expected_r64, tol, bug_id)
        character(len=*), intent(in) :: name
        type(FP16), intent(in) :: got_fp16
        real(real64), intent(in) :: expected_r64
        real(real64), intent(in) :: tol
...
END MODULE lpf_fp16_test_utils