...
MODULE lpf_fp8_e4m3_test_utils
    USE iso_fortran_env, only: real32, real64
    USE LPF_FP8_E4M3
    IMPLICIT NONE

    PRIVATE
    PUBLIC :: check_fp8_e4m3_real64
    PUBLIC :: check_logical
    PUBLIC :: check_integer
    PUBLIC :: check_fp8_e4m3_known_bug
    PUBLIC :: check_integer_known_bug
    PUBLIC :: check_logical_known_bug
    PUBLIC :: test_summary
    PUBLIC :: FP8_E4M3_TOL
    PUBLIC :: FP8_E4M3_TOL_TIGHT
    PUBLIC :: FP8_E4M3_TOL_LOOSE
    PUBLIC :: FP8_E4M3_TOL_COARSE
    ! FP8 E4M3 has only 3 mantissa bits (machine epsilon = 0.125).
    ! Tolerances are much looser than fp16/bf16.
    real(real64), parameter :: FP8_E4M3_TOL      = 1.0d-1   ! 10%  - default
    real(real64), parameter :: FP8_E4M3_TOL_TIGHT = 5.0d-2   ! 5%   - for near-exact ops
    real(real64), parameter :: FP8_E4M3_TOL_LOOSE = 2.5d-1   ! 25%  - for coarse ops
    real(real64), parameter :: FP8_E4M3_TOL_COARSE = 5.0d-1   ! 50%  - for coarse ops
...
CONTAINS
    ...
    subroutine check_fp8_e4m3_real64(name, got_fp8_e4m3, expected_r64, tol)
        character(len=*), intent(in) :: name
        type(FP8_E4M3), intent(in) :: got_fp8_e4m3
        real(real64), intent(in) :: expected_r64
        real(real64), intent(in) :: tol

        real(real64) :: got_r64, diff, threshold

        got_r64 = dble(got_fp8_e4m3)

        if (abs(expected_r64) < 1.0d-2) then
            ! FP8 E4M3 smallest subnormal is ~0.002, use coarser absolute threshold
            threshold = 1.0d-2
            diff = abs(got_r64 - expected_r64)
        else
            threshold = max(abs(expected_r64) * tol, 1.0d-2)
            diff = abs(got_r64 - expected_r64)
        end if
    ...
    subroutine check_fp8_e4m3_known_bug(name, got_fp8_e4m3, expected_r64, tol, bug_id)
        character(len=*), intent(in) :: name
        type(FP8_E4M3), intent(in) :: got_fp8_e4m3
        real(real64), intent(in) :: expected_r64
        real(real64), intent(in) :: tol
...
END MODULE lpf_fp8_e4m3_test_utils
