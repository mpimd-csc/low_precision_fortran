...
CONTAINS
    ...
    SUBROUTINE run_tests(ok)
        ...
        expected(1, 2) = FP16(4.5_real32)
        expected(2, 2) = FP16(-7.5_real32)
        expected(3, 2) = FP16(10.5_real32)
        ...
        expected(1, 3) = FP16(5.242188_real32)
        expected(2, 3) = FP16(-3.656250_real32)
        expected(3, 3) = FP16(-1.0_real32)
        expected(4, 3) = FP16(2.0_real32)
        ...
        expected(1, 5) = FP16(-9.0_real32)
        expected(2, 5) = FP16(6.0_real32)
        expected(3, 5) = FP16(-1.400391_real32)
        ...
        do i = 1, NTEST
            ytemp = y(:, i)
            CALL axpy(n(i), alpha(i), x(:, i), incx(i), ytemp, incy(i))
            ...
            do k = 1, 10
                IF (abs(dble(ytemp(k)) - dble(expected(k, i))) > 1.0d-2 * max(abs(dble(expected(k, i))), 1.0d-5)) THEN
                    print "(A, I2, A, I3, A, I3, A, I3, A, I2, A, F10.6, A, F10.6)", &
                          "HAXPY   -- FAIL -- Testcase ", i, ": N = ", n(i), ", INCX = ", incx(i), &
            ...
            IF (lok) THEN
                print "(A, I2, A, I3, A, I3, A, I3)", &
                      "HAXPY   -- PASS -- Testcase ", i, ": N = ", n(i), ", INCX = ", incx(i), ", INCY = ", incy(i)
            END IF
        end do
    end SUBROUTINE
END PROGRAM check_haxpy
