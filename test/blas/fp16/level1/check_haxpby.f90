...
CONTAINS
    ...
    SUBROUTINE run_tests(ok)
        ...
        TYPE(FP16), dimension(NTEST) :: alpha
        TYPE(FP16), dimension(NTEST) :: beta
        TYPE(FP16), dimension(10, NTEST) :: x
        TYPE(FP16), dimension(10, NTEST) :: y
        TYPE(FP16), dimension(10, NTEST) :: expected
        TYPE(FP16), dimension(10) :: ytemp
        INTEGER :: k
        LOGICAL :: lok

        ! Alpha values
        alpha(1) = FP16(1.0_real32)
        alpha(2) = FP16(2.0_real32)
        alpha(3) = FP16(sqrt(2.0_real32))
        alpha(4) = FP16(0.0_real32)
        alpha(5) = FP16(-1.0_real32)
        alpha(6) = FP16(-2.0_real32)
        alpha(7) = FP16(0.0_real32)
        alpha(8) = FP16(2.0_real32)

        ! Beta values
        beta(1) = FP16(1.0_real32)
        beta(2) = FP16(-1.0_real32)
        beta(3) = FP16(0.0_real32)
        beta(4) = FP16(1.0_real32)
        beta(5) = FP16(-1.0_real32)
        beta(6) = FP16(0.0_real32)
        beta(7) = FP16(1.0_real32)
        beta(8) = FP16(-1.0_real32)
        ...
        ! Expected results
        expected(1, 1) = FP16(2.0_real32)
        expected(2, 1) = FP16(-4.0_real32)
        expected(3, 1) = FP16(6.0_real32)
        expected(4, 1) = FP16(-8.0_real32)
        expected(5, 1) = FP16(10.0_real32)
        ...
        expected(1, 2) = FP16(1.5_real32)
        expected(2, 2) = FP16(-2.5_real32)
        expected(3, 2) = FP16(3.5_real32)
        ...
        expected(1, 3) = FP16(4.242188_real32)
        expected(2, 3) = FP16(-5.656250_real32)
        expected(3, 3) = FP16(0.0_real32)
        expected(4, 3) = FP16(0.0_real32)
        expected(5, 3) = FP16(1.0_real32)
        expected(6, 3) = FP16(2.0_real32)
        expected(7, 3) = FP16(-1.0_real32)
        expected(8, 3) = FP16(2.0_real32)
        expected(9, 3) = FP16(1.0_real32)
        expected(10, 3) = FP16(2.0_real32)
        expected(1, 4) = FP16(0.0_real32)
        expected(2, 4) = FP16(3.0_real32)
        expected(3, 4) = FP16(3.0_real32)
        expected(4, 4) = FP16(0.0_real32)
        expected(5, 4) = FP16(0.0_real32)
        expected(6, 4) = FP16(0.0_real32)
        expected(7, 4) = FP16(0.0_real32)
        expected(8, 4) = FP16(0.0_real32)
        expected(9, 4) = FP16(0.0_real32)
        expected(10, 4) = FP16(0.0_real32)
        expected(1, 5) = FP16(-11.0_real32)
        expected(2, 5) = FP16(2.0_real32)
        expected(3, 5) = FP16(0.600098_real32)
        expected(4, 5) = FP16(-2.0_real32)
        expected(5, 5) = FP16(1.0_real32)
        expected(6, 5) = FP16(2.0_real32)
        expected(7, 5) = FP16(-1.0_real32)
        expected(8, 5) = FP16(2.0_real32)
        expected(9, 5) = FP16(1.0_real32)
        expected(10, 5) = FP16(2.0_real32)
        expected(1, 6) = FP16(-18.0_real32)
        expected(2, 6) = FP16(9.0_real32)
        expected(3, 6) = FP16(-18.0_real32)
        expected(4, 6) = FP16(9.0_real32)
        expected(5, 6) = FP16(-18.0_real32)
        ...
        expected(1, 7) = FP16(1.0_real32)
        expected(2, 7) = FP16(2.0_real32)
        expected(3, 7) = FP16(-1.0_real32)
        expected(4, 7) = FP16(2.0_real32)
        expected(5, 7) = FP16(1.0_real32)
        expected(6, 7) = FP16(2.0_real32)
        expected(7, 7) = FP16(-1.0_real32)
        expected(8, 7) = FP16(2.0_real32)
        expected(9, 7) = FP16(1.0_real32)
        expected(10, 7) = FP16(2.0_real32)
        expected(1, 8) = FP16(10.0_real32)
        expected(2, 8) = FP16(4.0_real32)
        expected(3, 8) = FP16(-4.0_real32)
        expected(4, 8) = FP16(-5.0_real32)
        expected(5, 8) = FP16(0.399902_real32)
        expected(6, 8) = FP16(12.0_real32)
        expected(7, 8) = FP16(0.0_real32)
        expected(8, 8) = FP16(0.0_real32)
        expected(9, 8) = FP16(0.0_real32)
        expected(10, 8) = FP16(0.0_real32)

        do i = 1, NTEST
            ytemp = y(:, i)
            CALL axpby(n(i), alpha(i), x(:, i), incx(i), beta(i), ytemp, incy(i))
            ...
            do k = 1, 10
                IF (abs(dble(ytemp(k)) - dble(expected(k, i))) > 1.0d-2 * max(abs(dble(expected(k, i))), 1.0d-5)) THEN
                    print "(A, I2, A, I3, A, I3, A, I3, A, I2, A, F10.6, A, F10.6)", &
                          "HAXPBY  -- FAIL -- Testcase ", i, ": N = ", n(i), ", INCX = ", incx(i), &
            ...
            IF (lok) THEN
                print "(A, I2, A, I3, A, I3, A, I3)", &
                      "HAXPBY  -- PASS -- Testcase ", i, ": N = ", n(i), ", INCX = ", incx(i), ", INCY = ", incy(i)
            END IF
        end do
    end SUBROUTINE
END PROGRAM check_haxpby
