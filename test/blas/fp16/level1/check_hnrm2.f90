...
CONTAINS
    ...
    SUBROUTINE run_tests(ok)
        ...
        TYPE(FP16), dimension(NTEST) :: hnrm2_expected
        TYPE(FP16) :: result
        ...
        hnrm2_expected(1) = FP16(7.437500_real32)
        hnrm2_expected(2) = FP16(4.5552_real32)
        hnrm2_expected(3) = FP16(0.0_real32)
        hnrm2_expected(4) = FP16(2.0_real32)
        hnrm2_expected(5) = FP16(10.0_real32)
        hnrm2_expected(6) = FP16(0.0_real32)

        do i = 1, NTEST
            result = nrm2(n(i), data(:, i), incx(i))
            if (abs(dble(result) - dble(hnrm2_expected(i))) < 1.0d-2) then
                print "(A, I2, A, I3, A, I3, A, F10.6, A, F10.6)", &
                      "HNRM2   -- PASS -- Testcase ", i, ": N = ", n(i), ", INCX = ", incx(i), &
                      ", Result = ", dble(result), ", Expected = ", dble(hnrm2_expected(i))
            else
                print "(A, I2, A, I3, A, I3, A, F10.6, A, F10.6)", &
                      "HNRM2   -- FAIL -- Testcase ", i, ": N = ", n(i), ", INCX = ", incx(i), &
                      ", Result = ", dble(result), ", Expected = ", dble(hnrm2_expected(i))
                ok = .FALSE.
            end if
        end do
    end SUBROUTINE
END PROGRAM check_hnrm2
