...
void LPF_GLOBAL(hscal,HSCAL)(lpf_blas_int_t *, lpf_float16_t *, lpf_float16_t *, lpf_blas_int_t *);

// Testfunktion check_hnrm2
void check_hscal( bool *ok) {
#define NTEST 6
    ...
    lpf_float16_t sa [ NTEST ] = { 0.1, 0.0625, 1, -4, 10, 0};
    lpf_float16_t hscal_expected[NTEST][10] = {
    ...
    for ( int i = 0; i < NTEST; i++) {
        bool lok = true;
        LPF_GLOBAL(hscal, HSCAL)(&n[i], &sa[i], data[i], &incx[i]);
