...
!> \verbatim
!>
!> Test program for the FP16 Level 2 Blas.
!>
...
!> 'HBLAT2.SNAP'     NAME OF SNAPSHOT OUTPUT FILE
...
!> HGEMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> HGBMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> HSYMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> HSBMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> HSPMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> HTRMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> HTBMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> HTPMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> HTRSV  T PUT F FOR NO TEST. SAME COLUMNS.
!> HTBSV  T PUT F FOR NO TEST. SAME COLUMNS.
!> HTPSV  T PUT F FOR NO TEST. SAME COLUMNS.
!> HGER   T PUT F FOR NO TEST. SAME COLUMNS.
!> HSYR   T PUT F FOR NO TEST. SAME COLUMNS.
...
!     .. Local Arrays ..
      TYPE(FP16)               A( NMAX, NMAX ), AA( NMAX*NMAX ),              &
...
!     .. External Subroutines ..
      EXTERNAL           HCHK1, HCHK2, HCHK3, HCHK4, HCHK5, HCHK6,      &
     &                   HCHKE, HMVCH
...
!     .. Data statements ..
      DATA               SNAMES/'HGEMV ', 'HGBMV ', 'HSYMV ', 'HSBMV ', &
     &                   'HSPMV ', 'HTRMV ', 'HTBMV ', 'HTPMV ',        &
     &                   'HTRSV ', 'HTBSV ', 'HTPSV ', 'HGER  ',        &
     &                   'HSYR  ', 'HSPR  ', 'HSYR2 ', 'HSPR2 '/
...
!
!     Check the reliability of HMVCH using exact data.
!
      N = MIN( 16, NMAX )
      DO 120 J = 1, N
         DO 110 I = 1, N
...
!           Test error exits.
            IF( TSTERR )THEN
               CALL HCHKE( ISNUM, SNAMES( ISNUM ), NOUT )
               WRITE( NOUT, FMT = * )
            END IF
!           Test computations.
            INFOT = 0
     ...
     &              190, 190 )ISNUM
!           Test HGEMV, 01, and HGBMV, 02.
  140       CALL HCHK1( SNAMES( ISNUM ), EPS, THRESH, NOUT, NTRA, TRACE,&
     &                  REWI, FATAL, NIDIM, IDIM, NKB, KB, NALF, ALF,   &
     &                  NBET, BET, NINC, INC, NMAX, INCMAX, A, AA, AS,  &
     &                  X, XX, XS, Y, YY, YS, YT, G )
            GO TO 200
!           Test HSYMV, 03, HSBMV, 04, and HSPMV, 05.
  150       CALL HCHK2( SNAMES( ISNUM ), EPS, THRESH, NOUT, NTRA, TRACE,&
     &                  REWI, FATAL, NIDIM, IDIM, NKB, KB, NALF, ALF,   &
     &                  NBET, BET, NINC, INC, NMAX, INCMAX, A, AA, AS,  &
     &                  X, XX, XS, Y, YY, YS, YT, G )
            GO TO 200
!           Test HTRMV, 06, HTBMV, 07, HTPMV, 08,
!           HTRSV, 09, HTBSV, 10, and HTPSV, 11.
  160       CALL HCHK3( SNAMES( ISNUM ), EPS, THRESH, NOUT, NTRA, TRACE,&
     &                  REWI, FATAL, NIDIM, IDIM, NKB, KB, NINC, INC,   &
     &                  NMAX, INCMAX, A, AA, AS, Y, YY, YS, YT, G, Z )
            GO TO 200
!           Test HGER, 12.
  170       CALL HCHK4( SNAMES( ISNUM ), EPS, THRESH, NOUT, NTRA, TRACE,&
     &                  REWI, FATAL, NIDIM, IDIM, NALF, ALF, NINC, INC, &
     &                  NMAX, INCMAX, A, AA, AS, X, XX, XS, Y, YY, YS,  &
     &                  YT, G, Z )
            GO TO 200
!           Test HSYR, 13, and HSPR, 14.
  180       CALL HCHK5( SNAMES( ISNUM ), EPS, THRESH, NOUT, NTRA, TRACE,&
     &                  REWI, FATAL, NIDIM, IDIM, NALF, ALF, NINC, INC, &
     &                  NMAX, INCMAX, A, AA, AS, X, XX, XS, Y, YY, YS,  &
     &                  YT, G, Z )
            GO TO 200
!           Test HSYR2, 15, and HSPR2, 16.
  190       CALL HCHK6( SNAMES( ISNUM ), EPS, THRESH, NOUT, NTRA, TRACE,&
     &                  REWI, FATAL, NIDIM, IDIM, NALF, ALF, NINC, INC, &
     &                  NMAX, INCMAX, A, AA, AS, X, XX, XS, Y, YY, YS,  &
     &                  YT, G, Z )
!
...
!
 ...
 9994 FORMAT( ' ABSOLUTE VALUE OF INCX OR INCY IS 0 OR GREATER THAN ',  &
     &      I2 )
 9993 FORMAT( ' TESTS OF THE FP16             LEVEL 2 BLAS', //' THE F',&
 ...
 9986 FORMAT( ' SUBPROGRAM NAME ', A6, ' NOT RECOGNIZED', /' ******* T',&
     &      'ESTS ABANDONED *******' )
 9985 FORMAT( ' ERROR IN HMVCH -  IN-LINE DOT PRODUCTS ARE BEING EVALU',&
     &      'ATED WRONGLY.', /' HMVCH WAS CALLED WITH TRANS = ', A1,    &
     &      ' AND RETURNED SAME = ', L1, ' AND ERR = ', DT(12,3), '.', /   &
...
!
      END
      SUBROUTINE HCHK1( SNAME, EPS, THRESH, NOUT, NTRA, TRACE, REWI,    &
...
!
               TRANSL = ZERO
               CALL HMAKE( SNAME( 2: 3 ), ' ', ' ', M, N, A, NMAX, AA,  &
...
!
 ...
 9995 FORMAT( 1X, I6, ': ', A6, '(''', A1, ''',', 4( I3, ',' ), DT(4,1),   &
     &      ', A,', I3, ', X,', I2, ',', DT(4,1), ', Y,', I2, ') .' )
 9994 FORMAT( 1X, I6, ': ', A6, '(''', A1, ''',', 2( I3, ',' ), DT(4,1),   &
     &      ', A,', I3, ', X,', I2, ',', DT(4,1), ', Y,', I2,              &
     &      ')         .' )
...
!
      END
      SUBROUTINE HCHK2( SNAME, EPS, THRESH, NOUT, NTRA, TRACE, REWI,    &
     &                  FATAL, NIDIM, IDIM, NKB, KB, NALF, ALF, NBET,   &
     &                  BET, NINC, INC, NMAX, INCMAX, A, AA, AS, X, XX, &
     &                  XS, Y, YY, YS, YT, G )
        USE LPF_FP16
...
!
               TRANSL = ZERO
               CALL HMAKE( SNAME( 2: 3 ), UPLO, ' ', N, N, A, NMAX, AA, &
...
!                             Check the result.
!
                              CALL HMVCH( 'N', N, N, ALPHA, A, NMAX, X, &
...
!
 ...
 9997 FORMAT( ' ', A6, ' COMPLETED THE COMPUTATIONAL TESTS (', I6, ' C',&
     ...
     &      ' - SUSPECT *******' )
 9996 FORMAT( ' ******* ', A6, ' FAILED ON CALL NUMBER:' )
 9995 FORMAT( 1X, I6, ': ', A6, '(''', A1, ''',', I3, ',', DT(4,1), ', AP',&
     &      ', X,', I2, ',', DT(4,1), ', Y,', I2, ')                .' )
 9994 FORMAT( 1X, I6, ': ', A6, '(''', A1, ''',', 2( I3, ',' ), DT(4,1),   &
     &      ', A,', I3, ', X,', I2, ',', DT(4,1), ', Y,', I2,              &
     &      ')         .' )
 9993 FORMAT( 1X, I6, ': ', A6, '(''', A1, ''',', I3, ',', DT(4,1), ', A,',&
     &      I3, ', X,', I2, ',', DT(4,1), ', Y,', I2, ')             .' )
...
!
      END
      SUBROUTINE HCHK3( SNAME, EPS, THRESH, NOUT, NTRA, TRACE, REWI,    &
...
!
                     TRANSL = ZERO
                     CALL HMAKE( SNAME( 2: 3 ), UPLO, DIAG, N, N, A,    &
...
!                             Check the result.
!
                              CALL HMVCH( TRANS, N, N, ONE, A, NMAX, X, &
...
!
     ...
     &                              = X( I )
   50                         CONTINUE
                              CALL HMVCH( TRANS, N, N, ONE, A, NMAX, Z, &
...
!
      END
      SUBROUTINE HCHK4( SNAME, EPS, THRESH, NOUT, NTRA, TRACE, REWI,    &
...
!
                     TRANSL = ZERO
                     CALL HMAKE( SNAME( 2: 3 ), ' ', ' ', M, N, A, NMAX,&
...
!
 ...
 9994 FORMAT( 1X, I6, ': ', A6, '(', 2( I3, ',' ), DT(4,1), ', X,', I2,    &
     &      ', Y,', I2, ', A,', I3, ')                  .' )
 9993 FORMAT( ' ******* FATAL ERROR - ERROR-EXIT TAKEN ON VALID CALL *',&
     &      '******' )
!
...
!
      END
      SUBROUTINE HCHK5( SNAME, EPS, THRESH, NOUT, NTRA, TRACE, REWI,    &
     &                  FATAL, NIDIM, IDIM, NALF, ALF, NINC, INC, NMAX, &
     &                  INCMAX, A, AA, AS, X, XX, XS, Y, YY, YS, YT, G, &
     &                  Z )
        USE LPF_FP16
...
!
                  TRANSL = ZERO
                  CALL HMAKE( SNAME( 2: 3 ), UPLO, ' ', N, N, A, NMAX,  &
...
!
 ...
 9996 FORMAT( ' ******* ', A6, ' FAILED ON CALL NUMBER:' )
 9995 FORMAT( '      THESE ARE THE RESULTS FOR COLUMN ', I3 )
 9994 FORMAT( 1X, I6, ': ', A6, '(''', A1, ''',', I3, ',', DT(4,1), ', X,',&
     &      I2, ', AP)                           .' )
 9993 FORMAT( 1X, I6, ': ', A6, '(''', A1, ''',', I3, ',', DT(4,1), ', X,',&
     &      I2, ', A,', I3, ')                        .' )
 9992 FORMAT( ' ******* FATAL ERROR - ERROR-EXIT TAKEN ON VALID CALL *',&
     &      '******' )
!
...
!
      END
      SUBROUTINE HCHK6( SNAME, EPS, THRESH, NOUT, NTRA, TRACE, REWI,    &
     &                  FATAL, NIDIM, IDIM, NALF, ALF, NINC, INC, NMAX, &
     &                  INCMAX, A, AA, AS, X, XX, XS, Y, YY, YS, YT, G, &
     &                  Z )
        USE LPF_FP16
...
!
                     TRANSL = ZERO
                     CALL HMAKE( SNAME( 2: 3 ), UPLO, ' ', N, N, A,     &
...
!     it is called, and is then tested and re-set by CHKXER.
     ...
     &        160 )ISNUM
   10 INFOT = 1
      CALL HGEMV( '/', 0, 0, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HGEMV( 'N', -1, 0, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 3
      CALL HGEMV( 'N', 0, -1, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 6
      CALL HGEMV( 'N', 2, 0, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 8
      CALL HGEMV( 'N', 0, 0, ALPHA, A, 1, X, 0, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 11
      CALL HGEMV( 'N', 0, 0, ALPHA, A, 1, X, 1, BETA, Y, 0 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      GO TO 170
   20 INFOT = 1
      CALL HGBMV( '/', 0, 0, 0, 0, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HGBMV( 'N', -1, 0, 0, 0, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 3
      CALL HGBMV( 'N', 0, -1, 0, 0, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 4
      CALL HGBMV( 'N', 0, 0, -1, 0, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 5
      CALL HGBMV( 'N', 2, 0, 0, -1, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 8
      CALL HGBMV( 'N', 0, 0, 1, 0, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 10
      CALL HGBMV( 'N', 0, 0, 0, 0, ALPHA, A, 1, X, 0, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 13
      CALL HGBMV( 'N', 0, 0, 0, 0, ALPHA, A, 1, X, 1, BETA, Y, 0 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      GO TO 170
   30 INFOT = 1
      CALL HSYMV( '/', 0, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HSYMV( 'U', -1, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 5
      CALL HSYMV( 'U', 2, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 7
      CALL HSYMV( 'U', 0, ALPHA, A, 1, X, 0, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 10
      CALL HSYMV( 'U', 0, ALPHA, A, 1, X, 1, BETA, Y, 0 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      GO TO 170
   40 INFOT = 1
      CALL HSBMV( '/', 0, 0, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HSBMV( 'U', -1, 0, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 3
      CALL HSBMV( 'U', 0, -1, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 6
      CALL HSBMV( 'U', 0, 1, ALPHA, A, 1, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 8
      CALL HSBMV( 'U', 0, 0, ALPHA, A, 1, X, 0, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 11
      CALL HSBMV( 'U', 0, 0, ALPHA, A, 1, X, 1, BETA, Y, 0 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      GO TO 170
   50 INFOT = 1
      CALL HSPMV( '/', 0, ALPHA, A, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HSPMV( 'U', -1, ALPHA, A, X, 1, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 6
      CALL HSPMV( 'U', 0, ALPHA, A, X, 0, BETA, Y, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 9
      CALL HSPMV( 'U', 0, ALPHA, A, X, 1, BETA, Y, 0 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      GO TO 170
   60 INFOT = 1
      CALL HTRMV( '/', 'N', 'N', 0, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HTRMV( 'U', '/', 'N', 0, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 3
      CALL HTRMV( 'U', 'N', '/', 0, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 4
      CALL HTRMV( 'U', 'N', 'N', -1, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 6
      CALL HTRMV( 'U', 'N', 'N', 2, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 8
      CALL HTRMV( 'U', 'N', 'N', 0, A, 1, X, 0 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      GO TO 170
   70 INFOT = 1
      CALL HTBMV( '/', 'N', 'N', 0, 0, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HTBMV( 'U', '/', 'N', 0, 0, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 3
      CALL HTBMV( 'U', 'N', '/', 0, 0, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 4
      CALL HTBMV( 'U', 'N', 'N', -1, 0, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 5
      CALL HTBMV( 'U', 'N', 'N', 0, -1, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 7
      CALL HTBMV( 'U', 'N', 'N', 0, 1, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 9
      CALL HTBMV( 'U', 'N', 'N', 0, 0, A, 1, X, 0 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      GO TO 170
   80 INFOT = 1
      CALL HTPMV( '/', 'N', 'N', 0, A, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HTPMV( 'U', '/', 'N', 0, A, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 3
      CALL HTPMV( 'U', 'N', '/', 0, A, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 4
      CALL HTPMV( 'U', 'N', 'N', -1, A, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 7
      CALL HTPMV( 'U', 'N', 'N', 0, A, X, 0 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      GO TO 170
   90 INFOT = 1
      CALL HTRSV( '/', 'N', 'N', 0, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HTRSV( 'U', '/', 'N', 0, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 3
      CALL HTRSV( 'U', 'N', '/', 0, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 4
      CALL HTRSV( 'U', 'N', 'N', -1, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 6
      CALL HTRSV( 'U', 'N', 'N', 2, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 8
      CALL HTRSV( 'U', 'N', 'N', 0, A, 1, X, 0 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      GO TO 170
  100 INFOT = 1
      CALL HTBSV( '/', 'N', 'N', 0, 0, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HTBSV( 'U', '/', 'N', 0, 0, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 3
      CALL HTBSV( 'U', 'N', '/', 0, 0, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 4
      CALL HTBSV( 'U', 'N', 'N', -1, 0, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 5
      CALL HTBSV( 'U', 'N', 'N', 0, -1, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 7
      CALL HTBSV( 'U', 'N', 'N', 0, 1, A, 1, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 9
      CALL HTBSV( 'U', 'N', 'N', 0, 0, A, 1, X, 0 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      GO TO 170
  110 INFOT = 1
      CALL HTPSV( '/', 'N', 'N', 0, A, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HTPSV( 'U', '/', 'N', 0, A, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 3
      CALL HTPSV( 'U', 'N', '/', 0, A, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 4
      CALL HTPSV( 'U', 'N', 'N', -1, A, X, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 7
      CALL HTPSV( 'U', 'N', 'N', 0, A, X, 0 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      GO TO 170
  120 INFOT = 1
      CALL HGER( -1, 0, ALPHA, X, 1, Y, 1, A, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HGER( 0, -1, ALPHA, X, 1, Y, 1, A, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 5
      CALL HGER( 0, 0, ALPHA, X, 0, Y, 1, A, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 7
      CALL HGER( 0, 0, ALPHA, X, 1, Y, 0, A, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 9
      CALL HGER( 2, 0, ALPHA, X, 1, Y, 1, A, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      GO TO 170
  130 INFOT = 1
      CALL HSYR( '/', 0, ALPHA, X, 1, A, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HSYR( 'U', -1, ALPHA, X, 1, A, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 5
      CALL HSYR( 'U', 0, ALPHA, X, 0, A, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 7
      CALL HSYR( 'U', 2, ALPHA, X, 1, A, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      GO TO 170
  140 INFOT = 1
      CALL HSPR( '/', 0, ALPHA, X, 1, A )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HSPR( 'U', -1, ALPHA, X, 1, A )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 5
      CALL HSPR( 'U', 0, ALPHA, X, 0, A )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      GO TO 170
  150 INFOT = 1
      CALL HSYR2( '/', 0, ALPHA, X, 1, Y, 1, A, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HSYR2( 'U', -1, ALPHA, X, 1, Y, 1, A, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 5
      CALL HSYR2( 'U', 0, ALPHA, X, 0, Y, 1, A, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 7
      CALL HSYR2( 'U', 0, ALPHA, X, 1, Y, 0, A, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 9
      CALL HSYR2( 'U', 2, ALPHA, X, 1, Y, 1, A, 1 )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      GO TO 170
  160 INFOT = 1
      CALL HSPR2( '/', 0, ALPHA, X, 1, Y, 1, A )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 2
      CALL HSPR2( 'U', -1, ALPHA, X, 1, Y, 1, A )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 5
      CALL HSPR2( 'U', 0, ALPHA, X, 0, Y, 1, A )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
      INFOT = 7
      CALL HSPR2( 'U', 0, ALPHA, X, 1, Y, 0, A )
      CALL CHKXER( SRNAMT, INFOT, NOUT, LERR, OK )
!
  170 IF( OK )THEN
