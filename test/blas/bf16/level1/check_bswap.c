...
void LPF_GLOBAL(bswap,BSWAP)(lpf_blas_int_t *, lpf_bfloat16_t *, lpf_blas_int_t *, lpf_bfloat16_t *, lpf_blas_int_t *);

void check_bswap( bool *ok) {
#define NTEST 8
    ...
    for ( int i = 0; i < NTEST; i++) {
        ...
        LPF_GLOBAL(bswap, BSWAP)(&n[i], xtemp, &incx[i], ytemp, &incy[i]);
