...
CONTAINS
    ...
    SUBROUTINE run_tests(ok)
        ...
        TYPE(BF16), dimension(NTEST) :: basum_expected
        TYPE(BF16) :: result
        ...
        basum_expected(1) = BF16(15.0_real32)
        basum_expected(2) = BF16(7.5_real32)
        basum_expected(3) = BF16(0.0_real32)
        basum_expected(4) = BF16(4.0_real32)
        basum_expected(5) = BF16(10.0_real32)
        basum_expected(6) = BF16(0.0_real32)

        do i = 1, NTEST
            result = asum(n(i), data(:, i), incx(i))
            if (abs(dble(result) - dble(basum_expected(i))) < 1.0d-2) then
                print "(A, I2, A, I3, A, I3, A, F10.6, A, F10.6)", &
                      "HASUM   -- PASS -- Testcase ", i, ": N = ", n(i), ", INCX = ", incx(i), &
                      ", Result = ", dble(result), ", Expected = ", dble(basum_expected(i))
            else
                print "(A, I2, A, I3, A, I3, A, F10.6, A, F10.6)", &
