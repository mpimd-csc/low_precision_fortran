...
!> \verbatim
!>
!> Test program for the BF16 Level 2 Blas.
!>
...
!> BGEMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> BGBMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> BSYMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> BSBMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> BSPMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> BTRMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> BTBMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> BTPMV  T PUT F FOR NO TEST. SAME COLUMNS.
!> BTRSV  T PUT F FOR NO TEST. SAME COLUMNS.
!> BTBSV  T PUT F FOR NO TEST. SAME COLUMNS.
!> BTPSV  T PUT F FOR NO TEST. SAME COLUMNS.
!> BGER   T PUT F FOR NO TEST. SAME COLUMNS.
!> BSYR   T PUT F FOR NO TEST. SAME COLUMNS.
!> BSPR   T PUT F FOR NO TEST. SAME COLUMNS.
!> BSYR2  T PUT F FOR NO TEST. SAME COLUMNS.
!> BSPR2  T PUT F FOR NO TEST. SAME COLUMNS.
!>
!> Further Details
!> ===============
!>
...
!     .. Local Arrays ..
      TYPE(BF16)               A( NMAX, NMAX ), AA( NMAX*NMAX ),              &
...
!     .. Data statements ..
      DATA               SNAMES/'BGEMV ', 'BGBMV ', 'BSYMV ', 'BSBMV ', &
     &                   'BSPMV ', 'BTRMV ', 'BTBMV ', 'BTPMV ',        &
     &                   'BTRSV ', 'BTBSV ', 'BTPSV ', 'BGER  ',        &
     &                   'BSYR  ', 'BSPR  ', 'BSYR2 ', 'BSPR2 '/
...
!
 ...
 9994 FORMAT( ' ABSOLUTE VALUE OF INCX OR INCY IS 0 OR GREATER THAN ',  &
     &      I2 )
 9993 FORMAT( ' TESTS OF THE BF16             LEVEL 2 BLAS', //' THE F',&
...
!     .. Scalar Arguments ..
     ...
     &                   NOUT, NTRA
      ...
      CHARACTER*6        SNAME
!     .. Array Arguments ..
      TYPE(BF16) ::      A( NMAX, NMAX ), AA( NMAX*NMAX ), ALF( NALF ), &
...
!
     ...
     &                              INCX, BETA, INCY
                                 IF( REWI )                             &
     &                              REWIND NTRA
                                 CALL BGBMV( TRANS, M, N, KL, KU, ALPHA,&
     &                                       AA, LDA, XX, INCX, BETA,   &
     &                                       YY, INCY )
                              END IF
!
...
!
     ...
     &      INCX, BETA, INCY
         IF( REWI )                                                     &
     &      REWIND NTRA
         CALL BGEMV( TRANS, M, N, ALPHA, AA, LDA, XX, INCX, BETA, YY,   &
     &      INCY )
      ELSE IF( BANDED )THEN
         IF( TRACE )                                                    &
     ...
     &      ALPHA, LDA, INCX, BETA, INCY
         IF( REWI )                                                     &
     &      REWIND NTRA
         CALL BGBMV( TRANS, M, N, KL, KU, ALPHA, AA, LDA, XX, INCX,     &
     &      BETA, YY, INCY )
      END IF
      NC = NC + 1
...
!
     ...
     &                           UPLO, N, ALPHA, LDA, INCX, BETA, INCY
                              IF( REWI )                                &
     &                           REWIND NTRA
                              CALL BSYMV( UPLO, N, ALPHA, AA, LDA, XX,  &
     &                                    INCX, BETA, YY, INCY )
                           ELSE IF( BANDED )THEN
                              IF( TRACE )                               &
     &                           WRITE( NTRA, FMT = 9994 )NC, SNAME,    &
     ...
     &                           UPLO, N, ALPHA, INCX, BETA, INCY
                              IF( REWI )                                &
     &                           REWIND NTRA
                              CALL BSPMV( UPLO, N, ALPHA, AA, XX, INCX, &
     &                                    BETA, YY, INCY )
                           END IF
!
!                          Check if error-exit was taken incorrectly.
...
!
!  Tests BTRMV, BTBMV, BTPMV, BTRSV, BTBSV and BTPSV.
!
!  Auxiliary routine for test program for Level 2 Blas.
!
!  -- Written on 10-August-1987.
...
!
     ...
     &                           UPLO, TRANS, DIAG, N, LDA, INCX
                              IF( REWI )                                &
     &                           REWIND NTRA
                              CALL BTRMV( UPLO, TRANS, DIAG, N, AA, LDA,&
     &                                    XX, INCX )
                           ELSE IF( BANDED )THEN
                              IF( TRACE )                               &
     &                           WRITE( NTRA, FMT = 9994 )NC, SNAME,    &
     &                           UPLO, TRANS, DIAG, N, K, LDA, INCX
                              IF( REWI )                                &
     &                           REWIND NTRA
                              CALL BTBMV( UPLO, TRANS, DIAG, N, K, AA,  &
     &                                    LDA, XX, INCX )
                           ELSE IF( PACKED )THEN
                              IF( TRACE )                               &
     &                           WRITE( NTRA, FMT = 9995 )NC, SNAME,    &
     &                           UPLO, TRANS, DIAG, N, INCX
                              IF( REWI )                                &
     &                           REWIND NTRA
                              CALL BTPMV( UPLO, TRANS, DIAG, N, AA, XX, &
     &                                    INCX )
                           END IF
     ...
     &                           UPLO, TRANS, DIAG, N, LDA, INCX
                              IF( REWI )                                &
     &                           REWIND NTRA
                              CALL BTRSV( UPLO, TRANS, DIAG, N, AA, LDA,&
     &                                    XX, INCX )
                           ELSE IF( BANDED )THEN
                              IF( TRACE )                               &
     &                           WRITE( NTRA, FMT = 9994 )NC, SNAME,    &
     &                           UPLO, TRANS, DIAG, N, K, LDA, INCX
                              IF( REWI )                                &
     &                           REWIND NTRA
                              CALL BTBSV( UPLO, TRANS, DIAG, N, K, AA,  &
     &                                    LDA, XX, INCX )
                           ELSE IF( PACKED )THEN
                              IF( TRACE )                               &
     &                           WRITE( NTRA, FMT = 9995 )NC, SNAME,    &
     &                           UPLO, TRANS, DIAG, N, INCX
                              IF( REWI )                                &
     &                           REWIND NTRA
                              CALL BTPSV( UPLO, TRANS, DIAG, N, AA, XX, &
     &                                    INCX )
                           END IF
                        END IF
!
...
!
     ...
     &                  ALPHA, INCX, INCY, LDA
                     IF( REWI )                                         &
     &                  REWIND NTRA
                     CALL BGER( M, N, ALPHA, XX, INCX, YY, INCY, AA,    &
     &                          LDA )
!
!                    Check if error-exit was taken incorrectly.
!
...
!
     ...
     &                  ALPHA, INCX, LDA
                     IF( REWI )                                         &
     &                  REWIND NTRA
                     CALL BSYR( UPLO, N, ALPHA, XX, INCX, AA, LDA )
                  ELSE IF( PACKED )THEN
                     IF( TRACE )                                        &
     ...
     &                  ALPHA, INCX
                     IF( REWI )                                         &
     &                  REWIND NTRA
                     CALL BSPR( UPLO, N, ALPHA, XX, INCX, AA )
                  END IF
!
!                 Check if error-exit was taken incorrectly.
!
...
!
     ...
     &                     ALPHA, INCX, INCY, LDA
                        IF( REWI )                                      &
     &                     REWIND NTRA
                        CALL BSYR2( UPLO, N, ALPHA, XX, INCX, YY, INCY, &
     &                              AA, LDA )
                     ELSE IF( PACKED )THEN
                        IF( TRACE )                                     &
     &                     WRITE( NTRA, FMT = 9994 )NC, SNAME, UPLO, N, &
     &                     ALPHA, INCX, INCY
                        IF( REWI )                                      &
     &                     REWIND NTRA
                        CALL BSPR2( UPLO, N, ALPHA, XX, INCX, YY, INCY, &
     &                              AA )
                     END IF
!
!                    Check if error-exit was taken incorrectly.
