...
contains
    ...
    subroutine test_typical(uplo)
        ...
        call spmv(uplo, n, alpha, ap, x, incx, beta, y, incy)
        ...
        do i = 1, 3
            block
                ...
                expected = 1.0_real64
                do j = 1, 3
                    ! For ap = 1.0, A(i,j) = 1.0
                    expected = expected + 2.0_real64 * dble(ap(1)) * dble(x(j))
                end do
                call check_dt_real64("spmv_typical_" // uplo, int(i,int64), y(i), expected, GENERIC_TOL)
            end block
        end do
    end subroutine
    ...
    subroutine test_edge()
        ...
        alpha = 1.0
        beta = 1.0
        call spmv('U', n, alpha, ap, x, incx, beta, y, incy)
    end subroutine
    ...
    subroutine test_stride()
        ...
        do i = 1, 2
            block
                ...
                expected = 1.0_real64
                do j = 1, 2
                    expected = expected + 1.0_real64 * dble(ap(1)) * dble(x(2*j-1))
                end do
                call check_dt_real64("spmv_stride", int(i,int64), y(2*i-1), expected, GENERIC_TOL)
            end block
        end do
    end subroutine

end program test_level2_spmv
