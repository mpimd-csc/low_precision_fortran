...
contains
    ...
    subroutine test_typical(uplo, trans, diag)
        ...
        call trsv(uplo, trans, diag, n, a, n, x, incx)

        do i = 1, 3
            call check_dt_real64("trsv_typical", int(i,int64), x(i), dble(x_old(i)), GENERIC_TOL)
        end do
    end subroutine
    ...
    subroutine test_stride(uplo, trans, diag)
        ...
        do i = 1, 2
            call check_dt_real64("trsv_stride", int(i,int64), x(2*i-1), dble(x_old(2*i-1)), GENERIC_TOL)
        end do
    end subroutine

end program test_level2_trsv
