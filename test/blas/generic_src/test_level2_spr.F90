...
contains
    ...
    subroutine test_typical(uplo)
        ...
        call spr(uplo, n, alpha, x, incx, ap)
        ...
        do i = 1, 3
            ...
            do j = js, je
                block
                    ...
                    expected = dble(ap_old(pos)) + dble(alpha) *  dble(x(i)) * dble(x(j))
                    call check_dt_real64("spr_typical_" // uplo, int(pos,int64), ap(pos), expected, GENERIC_TOL)
                end block
            end do
        end do
    end subroutine
    ...
    subroutine test_stride(uplo)
        ...
        ! Check result. For ap=1.0, result is 1.0 + 2.0 * x(i)*x(j)
        ! We just check a few elements.
        do i = 1, 3
            if ( uplo .eq. 'U') then
                js = 1
                je = i
            ...
            do j = js, je
                block
                    ...
                    expected = dble(ap_old(pos)) + dble(alpha) *  dble(x(2*i-1)) * dble(x(2*j-1))
                    call check_dt_real64("spr_stride_" // uplo, int(pos,int64), ap(pos), expected, GENERIC_TOL)
                end block
            end do
        end do
