...
/*
 * gen_lookup_tables.c - Lookup table generator for fp8_e5m2 math functions.
 *
 ...
 *   fp8_e5m2_from_float(func(fp8_e5m2_to_float(i)))
 *
 */
...
{
    fprintf(out, "/* Lookup table for fp8_e5m2 %s: table[i] = "
            "fp8_e5m2_from_float(%s(fp8_e5m2_to_float(i))) */\n",
            entry->name, entry->cname);
    fprintf(out, "static const uint8_t fp8_e5m2_%s_table[256] = {\n",
            entry->name);

    for (int i = 0; i < 256; i++) {
        fp8_e5m2_t val = (uint8_t)i;
        uint8_t result;

        if (fp8_e5m2_isnan(val)) {
            result = FP8_E5M2_GET_SIGN(val) ? FP8_E5M2_NAN_NEG1 : FP8_E5M2_NAN_POS1;
        } else {
            float f = fp8_e5m2_to_float(val);
            float r = entry->func(f);
            result = fp8_e5m2_from_float(r);
        }
...
{
    fprintf(out, "/* Lookup table for fp8_e5m2 %s: table[i] = "
            "fp8_e5m2_from_float(%s(fp8_e5m2_to_float(i), fp8_e5m2_to_float(i))) */\n",
            entry->name, entry->cname);
    fprintf(out, "static const uint8_t fp8_e5m2_%s_table[256*256] = {\n",
            entry->name);

    int cnt = 0;
    for (int i = 0; i < 256; i++) {
        for (int j = 0; j < 256; j++) {
            fp8_e5m2_t val1 = (uint8_t)i;
            fp8_e5m2_t val2 = (uint8_t)j;
            uint8_t result;

            if (fp8_e5m2_isnan(val1)) {
                result = FP8_E5M2_GET_SIGN(val1) ? FP8_E5M2_NAN_NEG1 : FP8_E5M2_NAN_POS1;
            } else if (fp8_e5m2_isnan(val2)) {
                result = FP8_E5M2_GET_SIGN(val2) ? FP8_E5M2_NAN_NEG1 : FP8_E5M2_NAN_POS1;
            }  else {
                float f1 = fp8_e5m2_to_float(val1);
                float f2 = fp8_e5m2_to_float(val2);
...
static void write_header(FILE *out)
{
    fprintf(out,
        ...
        " * Lesser General Public License for more details.\n"
        " */\n"
        "#ifndef LPF_FP8_E5M2_LUT_H\n"
        "#define LPF_FP8_E5M2_LUT_H\n\n"
        "#include <stdint.h>\n\n"
        "#ifdef __cplusplus\n"
        "extern \"C\" {\n"
        "#endif\n\n"
