submodule(lpf_lapack_fp16) lpf_lapack_geqrt_fp16
...
contains
    ...
    module subroutine geqrt( norm, m, n, nb, a, lda, t, ldt, work, info )
        ...
        if( info.ne.0 ) then
            call lpf_blas_xerbla( 'geqrt', -info )
            return
        end if
        !
        ...
        do i = 1, k,  nb
            ...
            if( use_recursive_qr ) then
                call geqrt3( norm, m-i+1, ib, a(i,i), lda, t(1,i), ldt,          &
                    &                    iinfo )
            else
                call geqrt2( norm, m-i+1, ib, a(i,i), lda, t(1,i), ldt,          &
                    &                    iinfo )
            end if
            if( i+ib.le.n ) then
                !
                ...
                !
                call larfb( 'l', 't', 'f', 'c', m-i+1, n-i-ib+1, ib,       &
                    &                   a( i, i ), lda, t( 1, i ), ldt,                &
