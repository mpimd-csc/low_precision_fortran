...
int16_t LPF_GLOBAL(hasum, HASUM)(int64_t* n, lpf_float16_t* sx, int64_t* incx)
{
    ...
    if (*incx == 1)
    {
        ...
        {
            stemp = stemp + abs_fp16(sx[i__]) + abs_fp16(sx[i__ + 1]) +
                    abs_fp16(sx[i__ + 2]) + abs_fp16(sx[i__ + 3]) +
                    abs_fp16(sx[i__ + 4]) + abs_fp16(sx[i__ + 5]);
        }
    }
    else
    {
...
lpf_ffloat16_t lpf_blas_hasum_fortran_dyn_rank_64(int64_t* n, CFI_cdesc_t* _sx,
                                                  int64_t* incx)
{
...
lpf_ffloat16_t lpf_blas_hasum_fortran_dyn_rank_32(int32_t* n, CFI_cdesc_t* _sx,
                                                  int32_t* incx)
{
    lpf_float16_t* sx = _sx->base_addr;
    int64_t _n = *n;
