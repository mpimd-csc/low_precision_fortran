...
/*       SUBROUTINE BTBMV(UPLO,TRANS,DIAG,N,K,A,LDA,X,INCX) */
...
/* > \verbatim */
/* > */
/* > BTBMV  performs one of the matrix-vector operations */
/* > */
/* >    x := A*x,   or   x := A**T*x, */
/* > */
...
void LPF_GLOBAL(btbmv,BTBMV)(char *uplo, char *trans, char *diag, int64_t *n,
        int64_t *k, lpf_bfloat16_t *a, int64_t *lda, lpf_bfloat16_t *x, int64_t *incx, lpf_fortran_strlen_t
        uplo_len, lpf_fortran_strlen_t trans_len, lpf_fortran_strlen_t diag_len)
{
    ...
    if (info != 0) {
        ...
        LPF_GLOBAL(lpf_blas_xerbla, LPF_BLAS_XERBLA)("BTBMV ", &infox, (lpf_fortran_strlen_t)6);
        return;
    }
...
void lpf_blas_btbmv_fortran_dyn_rank_64(char *uplo, char *trans, char *diag, int64_t *n,
        int64_t *k, CFI_cdesc_t *_a, int64_t *lda, CFI_cdesc_t *_x, int64_t *incx)
{
    lpf_bfloat16_t *a = _a->base_addr;
    lpf_bfloat16_t *x = _x->base_addr;
    LPF_GLOBAL(btbmv,BTBMV)(uplo, trans, diag, n,
        k, (lpf_bfloat16_t *)a, lda, (lpf_bfloat16_t *)x, incx, 1, 1, 1);
}

void lpf_blas_btbmv_fortran_dyn_rank_32(char *uplo, char *trans, char *diag, int32_t *n,
        int32_t *k, CFI_cdesc_t *_a, int32_t *lda, CFI_cdesc_t *_x, int32_t *incx)
{
    lpf_bfloat16_t *a = _a->base_addr;
    lpf_bfloat16_t *x = _x->base_addr;
    int64_t _n = *n;
    ...
    int64_t _lda = *lda;
    int64_t _incx = *incx;
    LPF_GLOBAL(btbmv,BTBMV)(uplo, trans, diag, &_n,
        &_k, (lpf_bfloat16_t *)a, &_lda, (lpf_bfloat16_t *)x, &_incx, 1, 1, 1);
}
