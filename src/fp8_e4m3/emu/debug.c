 ...
 *
 * Produces a string like "0000 0000", aligning with the FP8 E4M3 bit-field
 * layout: [S] [EEEE] [MMM].
 * Uses a thread-unsafe static buffer — not suitable for concurrent use.
 *
...
/**
 * Print a table of all 256 FP8 E4M3 encodings to stdout.
 *
 * Each line shows the hex code, the 8-bit binary representation (with
 * space separating sign/exponent/mantissa fields), and the float value
 * obtained via fp8_e4m3_to_float().  Useful for verification and
 * visual inspection of the full encoding space.
 */
void fp8_e4m3_dump_all()
{
    uint16_t i;
    for (i = 0; i < 256; ++i) {
        printf("FP8_E4M3 (0x%02x, %8s) = %10.8f\n", i, str_bit_8(i), fp8_e4m3_to_float(i));
    }
}
