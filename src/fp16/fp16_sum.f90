...
submodule (lpf_fp16) lpf_fp16_sum
    use iso_fortran_env
    implicit none
contains
            ...
            if ( present (mask) ) then
                select rank(mask)
                    ...
                    rank(2)
                        ...
                        if ( dim .eq. 1 ) then
                            do j = 1, size(vec,2)
                                out(j) = fp16_sum_r1(vec(:,j), mask(:, j))
                            end do
                     ...
                     rank(3)
                        if ( size(vec, 1) .ne. size(mask,1) .or. size(vec,2) .ne. size(mask, 2) &
                         ...
                         if ( dim .eq. 1 ) then
                            do i = 1, size(vec,2)
                                do j = 1, size(vec,3)
                                    out(i, j) = fp16_sum_r1(vec(:, i , j), mask(:,i,j))
                                end do
                            end do
                        else if ( dim .eq. 2 ) then
                            do i = 1, size(vec,1)
