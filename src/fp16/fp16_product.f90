...
submodule (lpf_fp16) lpf_fp16_product
    use iso_fortran_env
    implicit none
contains
        ...
        module pure function fp16_product_r2_dim(vec, dim, mask) result(out)
            ...
            if ( present (mask) ) then
                select rank(mask)
                    rank(0)
                        if ( mask ) then
                            out = fp16_product_r2_dim(vec, dim)
                        else
                            out = 1.0
                        end if
                    rank(2)
                        ...
                        if ( dim .eq. 1 ) then
                            do j = 1, size(vec,2)
                                out(j) = fp16_product_r1(vec(:,j), mask(:, j))
                            end do
        ...
        module pure function fp16_product_r3_dim(vec, dim, mask) result(out)
            ...
            if ( present (mask) ) then
                select rank(mask)
                    rank(0)
                        if ( mask ) then
                            out = fp16_product_r3_dim(vec, dim)
                        else
                            out = 1.0
                        end if
                     rank(3)
                        if ( size(vec, 1) .ne. size(mask,1) .or. size(vec,2) .ne. size(mask, 2) &
                         ...
                         if ( dim .eq. 1 ) then
                            do i = 1, size(vec,2)
                                do j = 1, size(vec,3)
                                    out(i, j) = fp16_product_r1(vec(:, i , j), mask(:,i,j))
                                end do
                            end do
                        else if ( dim .eq. 2 ) then
                            do i = 1, size(vec,1)
                ...
                if ( dim .eq. 1 ) then
                    do i = 1, size(vec,2)
                        do j = 1, size(vec,3)
                            out(i, j) = fp16_product_r1(vec(:, i , j))
                        end do
                    end do
                else if ( dim .eq. 2 ) then
                    do i = 1, size(vec,1)
