#ifndef LPF_FP16_TLAPACK_HPP
#define LPF_FP16_TLAPACK_HPP

#include <iostream>
#include <iomanip>
#include <stdexcept>

#include "lpf_fp16.hpp"
...
namespace tlapack {
    ...
    namespace traits {
        ...
        template <>
            struct real_type_traits<lpf_fp16_t, int> {
                using type = lpf_fp16_t;
                constexpr static bool is_real = true;
            };
        // The complex type of mpfr::mpreal is std::complex<mpfr::mpreal>
        template <>
            struct complex_type_traits<lpf_fp16_t, int> {
                using type = std::complex<lpf_fp16_t>;
    ...
    template <>
        inline int digits<lpf_fp16_t>() noexcept
        {
